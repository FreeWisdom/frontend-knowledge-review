...
- TS 是 JS 的一个超集，在原有的语法基础上，添加了可选静态类型和基于类的面向对象编程
- 面向项目：
...
// 自己的
function zhz(arg: Object): { toString(): string } {
  return arg; // OK
}
...
## 4、断言 - 类型的中间执行时声明、转换（过程中和编译器的交流）

```ts
// 尖括号形式
let anyValue: any = "zhz";
...
function zhz(target: Function): void {
